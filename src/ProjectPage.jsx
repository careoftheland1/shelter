...
function ProjectPage() {
  ...
  const go = index => { setErrors({}); setShowAnswers(false); if (index < 0) setReturnTo(null); setStep(index); firstChange.current = true; };
  const edit = index => {
    if (returnTo !== null && !validate()) return;
    if (index < step) setReturnTo(current => current ?? step);
    go(index);
  };
  ...
  const validationFor = index => {
    const current = steps[index];
    const next = {};
    if (current.choices && !(current.key === "timing" && answers.land === "Already building") && !answers[current.key]) next[current.key] = "Choose an answer to continue.";
  ...
  const focusInvalid = issues => requestAnimationFrame(() => {
    if (issues.link) document.querySelector(".funnel-add-link").open = true;
    document.querySelector(".funnel-panel [aria-invalid='true'], .funnel-panel input:not(:checked)")?.focus();
  });
  const validate = () => {
    const issues = validationFor(step);
    setErrors(issues);
    if (Object.keys(issues).length) { focusInvalid(issues); return false; }
    return true;
  };
  const next = e => {
    e.preventDefault();
    if (!validate()) return;
    if (returnTo !== null && step < returnTo) {
  ...
  const submit = async () => {
    ...
    for (let index = 0; index < steps.length; index++) {
      const issues = validationFor(index);
      if (Object.keys(issues).length) { go(index); setErrors(issues); focusInvalid(issues); return; }
    }
    setDeliveryError("");
    if (!endpoint) {
  ...
  const answeredSteps = steps.map((item, index) => ({ item, index })).filter(({ index }) => index < furthestStep && index !== step);
  const returnLabel = returnTo === steps.length ? "Return to review" : returnTo !== null ? `Return to ${steps[returnTo].label}` : "";
  return <main className="funnel-page project-page">
    ...
    {step >= 0 && step < steps.length && <div className="funnel-workspace">
      {(answeredSteps.length > 0 || returnTo !== null) && <div className="funnel-history"><button className="funnel-history-all" onClick={() => setShowAnswers(value => !value)} aria-expanded={showAnswers} aria-controls="funnel-answer-drawer">{showAnswers ? "Hide answered questions" : `Answered questions (${answeredSteps.length})`} <span>{showAnswers ? "−" : "+"}</span></button></div>}
      {(answeredSteps.length > 0 || returnTo !== null) && <div className="funnel-answer-drawer" id="funnel-answer-drawer" hidden={!showAnswers}>{answeredSteps.map(({ item, index }) => <button key={item.key} onClick={() => edit(index)}>{String(index + 1).padStart(2,"0")} / {item.label}<span>Edit ↗</span></button>)}{returnTo !== null && <button className="funnel-answer-return" onClick={() => { if (!validate()) return; const destination = returnTo; setReturnTo(null); go(destination); }}>{returnLabel}<span>↗</span></button>}</div>}
